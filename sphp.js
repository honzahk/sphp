...
const switchPhpA2 = phpTarget => {
  ...
  execSync(
    `${cmd_a2_disable_php_versions} sudo a2enmod php${phpTarget} && sudo service apache2 restart`
  );
};
...
const main = () => {
  ...
  if (
    !phpInstalled.cli.includes(phpTarget) ||
    !phpInstalled.a2.includes(phpTarget)
  ) {
